...
This test uses INT8 packed mode (sys_mode=2).
...
def compute_expected_outputs(A, W):
    ...
    A: M x N matrix of packed INT8 values
    W: N x N matrix of packed INT8 values
    ...
    for t in range(num_input_rows):
        ...
        for col in range(num_cols):
            ...
            for k in range(len(W)):  # N rows of W
                psum += pe_int8_compute(A[t][k], W[k][col])
...
async def test_int8_e2e(dut):
    """INT8 Packed Mode E2E Test - Full Scale Random Test for NxN Array"""
    
    # Configuration
    NUM_INPUT_ROWS = 50  # Number of rows to test
    SEED = 42
    
    dut._log.info(f"=== INT8 E2E Test with N={N} ===")
    ...
    A = generate_random_packed_matrix(NUM_INPUT_ROWS, N, range_min=-128, range_max=127, seed=SEED)
    W = generate_random_packed_matrix(N, N, range_min=-128, range_max=127, seed=SEED + 1)
        
    # Log the generated data
    dut._log.info(f"Input A ({len(A)} rows x {N} cols):")
    for i, row in enumerate(A[:4]):  # Show first 4 rows
        row_str = ", ".join([f"({unpack_int8(v)[0]},{unpack_int8(v)[1]})" for v in row])
