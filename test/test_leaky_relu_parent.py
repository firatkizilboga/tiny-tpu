...
async def test_leaky_relu_parent_4x2_staggered(dut):
    ...
    for cycle_num in range(10):
        if int(dut.lr_valid_1_out.value):
            output_val = from_fixed(int(dut.lr_data_1_out.value))
            col1_results.append(output_val)
            print(f"cycle {cycle_num}: col1 output = {output_val:.5f}")
            
        if int(dut.lr_valid_2_out.value):
            output_val = from_fixed(int(dut.lr_data_2_out.value))
...
async def test_leaky_relu_parent_invalid_inputs(dut):
    ...
    await RisingEdge(dut.clk)
    # assert int(dut.lr_valid_1_out.value) == 0, "col1 output should be invalid when lr_valid_1_in is low"
    # assert int(dut.lr_valid_2_out.value) == 0, "col2 output should be invalid when lr_valid_2_in is low"
    # assert int(dut.lr_data_1_out.value) == 0, "col1 output data should be zero when invalid"
    # assert int(dut.lr_data_2_out.value) == 0, "col2 output data should be zero when invalid"
    ...
    await RisingEdge(dut.clk)
    # assert int(dut.lr_valid_1_out.value) == 1, "col1 output should be valid when lr_valid_1_in is high"
    # assert int(dut.lr_valid_2_out.value) == 0, "col2 output should be invalid when lr_valid_2_in is low"
    ...
    await RisingEdge(dut.clk)
    # assert int(dut.lr_valid_1_out.value) == 0, "col1 output should be invalid when lr_valid_1_in is low"
    # assert int(dut.lr_valid_2_out.value) == 1, "col2 output should be valid when lr_valid_2_in is high"
