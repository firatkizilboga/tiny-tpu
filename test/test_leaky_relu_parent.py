...
async def monitor_outputs(dut, col1_results, col2_results):
    while True:
        ...
        if read_packed_valid(dut.lr_valid_out, 0):
            val = read_packed(dut.lr_data_out, 0, 16)
            col1_results.append(from_fixed(val))
            
        if read_packed_valid(dut.lr_valid_out, 1):
            val = read_packed(dut.lr_data_out, 1, 16)
...
async def test_leaky_relu_parent_4x2_staggered(dut):
    ...
    lr_valid_drv = PackedArrayDriver(dut.lr_valid_in, 1)
    lr_data_drv = PackedArrayDriver(dut.lr_data_in, 16)
    ...
    for i, (use_col1, use_col2) in enumerate(staggered_pattern):
        if use_col1 and col1_idx < len(BATCH_4x2_DATA_COL1):
            ...
            lr_data_drv.set(0, to_fixed(val))
            lr_valid_drv.set(0, 1)
            col1_idx += 1
        else:
            lr_data_drv.set(0, 0)
            lr_valid_drv.set(0, 0)
        
        if use_col2 and col2_idx < len(BATCH_4x2_DATA_COL2):
            val = BATCH_4x2_DATA_COL2[col2_idx]
            lr_data_drv.set(1, to_fixed(val))
            lr_valid_drv.set(1, 1)
            col2_idx += 1
        else:
            lr_data_drv.set(1, 0)
            lr_valid_drv.set(1, 0)
...
async def test_leaky_relu_parent_invalid_inputs(dut):
    ...
    dut.lr_leak_factor_in.value = to_fixed(LEAK_FACTOR)
    lr_data_drv.set(0, to_fixed(1.5))
    lr_data_drv.set(1, to_fixed(-2.0))
    
    lr_valid_drv.set(0, 0)
    lr_valid_drv.set(1, 0)
    await RisingEdge(dut.clk) 
    await Timer(1, "ps") # Wait for value to settle and exit ReadOnly
    assert read_packed_valid(dut.lr_valid_out, 0) == 0
    
    lr_valid_drv.set(0, 1)
    lr_valid_drv.set(1, 0)
    await RisingEdge(dut.clk)
    await Timer(1, "ps")
    assert read_packed_valid(dut.lr_valid_out, 0) == 1
    assert read_packed_valid(dut.lr_valid_out, 1) == 0
